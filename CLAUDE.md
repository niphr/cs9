...
Measured on 2026-08-15, cs9 26.8.18: that NOTE lists `callr`, `foreach`,
`future`, `later` and `progress`. Four of the five ARE used, each by a
`::` call inside an R6 method. Only `progress` is genuinely unused.
**The NOTE is therefore mostly a false positive and does not clear by
using the package more.** Read it as “check by hand”, not as “delete
these”.

The cost is not cosmetic. `Task$run()` called
[`future::plan()`](https://future.futureverse.org/reference/plan.html)
and
[`foreach::registerDoSEQ()`](https://rdrr.io/pkg/foreach/man/registerDoSEQ.html)
at `R/r6_Task.R:262` and `:263` for years with neither package declared.
`R CMD check` could not see the calls, and no test executed them, so
both safety nets were down at once. It surfaced only when `26.8.17`
added a test that drives the public `Task$run()` to completion.
...
Each one survives for the reason `future` did: every machine that runs
cs9 happens to have it.
