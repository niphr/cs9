...
The task creation workflow follows CS9’s single-instance design
principle. You write your analytical code as if it processes a single
scenario (one disease, one location, one time period). You then define
the iteration scope through plans. CS9 automatically expands your
single-instance code across all specified combinations. CS9 handles
parallelization and database operations transparently. This means you
can develop and debug interactively with real data for one case, then
deploy the same code across hundreds of combinations without
modification.
