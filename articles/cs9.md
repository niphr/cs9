...
## The problem: scaling epidemiological analysis

Epidemiologists naturally think about surveillance problems in terms of
specific scenarios: “How do I detect influenza outbreaks among elderly
patients in Oslo?” They develop understanding through particular cases
before generalizing.

But comprehensive surveillance requires scaling these analyses across
hundreds of diseases, locations, and demographic groups. Traditional
programming forces epidemiologists to manage this complexity explicitly,
burying analytical logic within nested loop structures:

``` r
# Traditional approach: epidemiological logic buried in scaling code
for (location in all_locations) {
  for (week in all_weeks) {
    for (age_group in all_age_groups) {
      data <- get_data(location, week, age_group)
      results <- analyze_disease_trends(data)
      save_results(results, location, week, age_group)
    }
  }
}
```

CS9 resolves this by **separating analytical logic from scaling**.
Epidemiologists write code for a single instance — one disease, one
location, one age group — while the framework handles expansion across
all combinations automatically:

``` r
# CS9 approach: write analysis for one instance, framework scales it
short_term_trends_action <- function(data, argset, tables) {
  x <- csalert::short_term_trend(data$data, ...)
  tables$output$insert_data(x)
}
```

This code operates identically whether run interactively for one
location or scaled by CS9 across 106 diseases, 378 locations, and
multiple demographic groups. When a specific analysis fails, the
epidemiologist loads that exact combination interactively and steps
through the code. This is the **single-instance design principle** —
CS9’s core innovation.

## NorSySS: a decade of operation

The Norwegian Syndromic Surveillance System (NorSySS) provides evidence
that this approach works in practice. Operating continuously since 2014,
NorSySS has:

- Processed approximately **18 million** healthcare consultations
  annually
- Monitored **106 diseases** across **378 geographic locations**
  (municipal to national)
- Maintained **99.5% system availability** over ten years
- Supported **9 age groups** and **3 sex categories** for demographic
  stratification
...
| Aspect                   | CS9                                | plnr                                     | Commercial Platforms         | R Frameworks                       | Custom Scripts             |
|--------------------------|------------------------------------|------------------------------------------|------------------------------|------------------------------------|----------------------------|
| Customization            | High — domain abstractions         | High — domain abstractions               | Low — vendor-limited         | Medium — computational flexibility | High — unlimited freedom   |
| Maintenance              | Low — systematic abstractions      | Low — systematic abstractions            | Medium — vendor dependencies | Medium — framework constraints     | High — technical debt      |
| User model               | Epidemiological concepts           | Epidemiological concepts                 | Standardized interfaces      | Computational tasks                | Complete freedom           |
| Implementation speed     | Medium — initial setup, then rapid | High                                     | High — immediate capability  | Medium — learning curve            | Variable                   |
...
rapid adaptation. General-purpose R workflow frameworks (targets, drake,
workflowr) provide computational reproducibility but require translating
surveillance concepts into computational abstractions. Custom scripts
offer freedom but accumulate technical debt.

## What is CS9?

Core Surveillance 9 (CS9) is an R framework for building real-time
disease surveillance systems. It provides infrastructure for:
...
CS9 is aimed at public health organizations, epidemiologists, and
researchers who need to process surveillance data on a regular,
automated schedule.
...
CS9 is not always the right choice. The framework repays its setup cost
when the analysis itself is complex: temporal calculations, integration
across several data sources, or novel analytical methods. Where the
analytical requirements are standard, that cost is not repaid.

Three requirements are hard:

- PostgreSQL, and containerised deployment.
- Software architecture and systems administration capability, in-house
  or through a partner.
- Workflow reorganisation. Adoption changes how epidemiologists work,
  and the productivity gain arrives over a medium-to-long horizon, not
  immediately.

Assess your analytical requirements, your technical capability and your
time horizon before committing. For a smaller system, a pilot, or a
resource-constrained setting, [plnr](https://www.rwhite.no/plnr/)
applies the same single-instance design on one computer with no
database. CS9 is built on plnr, and adds database integration and
multi-plan orchestration.

### What CS9 does not decide for you

**CS9 analyses each stratum independently by default.** That default
reflects NorSySS’s operational need for location-specific and
demographic-specific municipal reporting. It is a default, not a
methodological commitment.

Borrowing strength across strata, and adjusting exceedance probabilities
for multiple comparisons, are decisions about the statistical method.
They are independent of the infrastructure, and you can implement either
inside an action function. With 106 diseases across 378 locations and
several demographic strata, the number of simultaneous tests is large.
Whether to control the family-wise error rate or the false discovery
rate, and how, is your decision. The framework does not make it for you,
and does not warn you that it has not.
...
- **Tasks** are the unit of scheduled work — things like “download
  weather data”, “calculate disease trends”, or “generate reports”.
- **Plans** are data processing units within a task, typically organized
  by time period, geographic area, or data subset.
- **Analyses** are the individual computations within a plan — the
  actual work performed on the extracted data.
...
2.  **Data cleaning**: Validate, standardize, and quality-check incoming
    data
3.  **Analysis**: Calculate surveillance indicators, trends, or alerts
4.  **Reporting**: Generate outputs such as Excel files, plots, or
    automated reports
5.  **Delivery**: Send results by email, upload to a website, or trigger
    alerts

![](cs9-task-general.png)

Figure 1. A general task showing the many options of a task.

[Figure 1](#figure-1) shows the available options within a task.

Data can be read from any source. Within a plan, `data_selector_fn`
extracts the data **once** (a single data-pull). That data is then
passed to each analysis, which calls `action_fn` with:
...
A plan-heavy task has many plans with few analyses each. An
analysis-heavy task has few plans with many analyses each.

Data-pulls are relatively slow, so fewer plans means less total time
waiting for data. If each data-pull extracts a larger dataset, the
analyses can subset it as needed (via argsets). Analysis-heavy tasks are
therefore generally faster, at the cost of higher RAM usage.

If RAM is limited, use more plans with smaller data-pulls instead.

[Figure 1](#figure-1) shows only 2 location-based analyses. In practice,
Norway had 356 municipalities in 2021. If [figure 1](#figure-1) had 2
plans (one for 2021 data, one for 2020 data) with 356 analyses each (one
per `location_code`), that would be an analysis-heavy approach.

## Other vignettes

- **Installation and setup**:
  [`vignette("installation")`](https://niphr.github.io/cs9/articles/installation.md)
  covers database configuration and environment variables
- **Package structure**:
  [`vignette("file-layout")`](https://niphr.github.io/cs9/articles/file-layout.md)
  explains how to organize your implementation files
- **Creating tasks**:
  [`vignette("creating-a-task")`](https://niphr.github.io/cs9/articles/creating-a-task.md)
  walks through building your first surveillance task step by step
...
Start by creating a surveillance system. This object coordinates tables
and tasks.
...
As documented in more detail
[here](https://niphr.github.io/csdb/reference/DBTable_v9.html), we
create a database table for weather data and add it to the surveillance
system.
...
`for_each_plan` expects a list. Each element corresponds to one plan;
its values are added to the argset for every analysis inside that plan.

The following code produces 4 plans with 1 analysis each, where each
analysis receives `argset$var_1` and `argset$var_2`:
...
works like `expand.grid`, but returns a list instead of a data frame.
...
`for_each_plan` generates `length(for_each_plan)` plans.
`for_each_analysis` works the same way, but generates **analyses**
within each plan.

#### universal_argset

A named list whose values are added to the argset of every analysis in
the task.

#### upsert_at_end_of_each_plan

When `TRUE` and `tables` contains a table named `output`, the return
value of `action_fn` is upserted to `tables$output` at the end of each
**plan**.

When `TRUE` and `action_fn` returns a named list, each element is
upserted to the corresponding `tables$NAME_FROM_LIST` at the end of each
**plan**.

If you upsert or insert manually from within `action_fn`, you can only
do so at the end of each **analysis**.

#### insert_at_end_of_each_plan

Same as `upsert_at_end_of_each_plan`, but inserts instead of upserts.

If you insert manually from within `action_fn`, you can only do so at
the end of each **analysis**.

#### action_fn_name

A character string naming the action function, preferably including the
package name.

#### data_selector_fn_name

A character string naming the data selector function, preferably
...
The block inside `if(plnr::is_run_directly()){` is for interactive
development. Running the function manually executes that block, which
loads `argset` and `tables` into your environment so you can step
...
The `if(plnr::is_run_directly()){` block works the same way here:
running the function manually loads `data`, `argset`, and `tables` so
you can work through the analysis interactively.
...
![](cs9-task-export-multiple-results.png)
...
![](cs9-task-export-combined-results.png)
...
2.  **Testing**: Run individual tasks with
    `global$ss$run_task("task_name")`
...
- **Structured logging**, at the level of the individual analysis:
  execution time, data volume processed, and which
  disease-location-demographic combination threw an error.
- **Database management**: schema validation, partitioning by time
  period, and bulk operations over millions of rows. Partitioning holds
  query performance steady as history accumulates over years. Data
  selectors keep the database structure out of the analysis code.
- **Parallel computation**, at the framework level. You write sequential
  R code. The same code runs on a laptop with 4 cores during development
  and a server with 20 cores in production.
- **Validation workflows**: completeness, temporal consistency, and
...
provides a working surveillance system you can adapt:
...
- [Installation](https://niphr.github.io/cs9/articles/installation.md) —
...
- [File layout](https://niphr.github.io/cs9/articles/file-layout.md) —
  how to organize your package files
